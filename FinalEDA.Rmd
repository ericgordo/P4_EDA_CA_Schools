...
Now that we have seen some distributions of individual values in our dataset, lets start to compare these values with each other to see if any of these variables have high correlations. In order to first just take a glimpse of comparing all of the data, we will use the ggcorr function to plot multiple variables against each other at once. Before I print this, some things I wonder about the data are:
...
*After  this grid of correlations, some interesting results are worth noting. First, there does in fact seem to be many highly linear correlations in this data, which we will see is in some casses very surprising. Second, it seems that the DTOT, or "Total Drop Outs" for schools has the one of the weakest correlations to all other vatiables, which is surprising to me because the the number of dropouts a school has seem to almost always be a large red flag for other issues at schools, which I would assume would mean lower test scores as well. However the small negative correlations seem to suggest that there is only a minor correlation with drop outs. 
...
To test this model, we will use the actual averages from one specific school, and input it into our linear model, then compare our result with the actual school's Average SAT Math Score. 
...
print("Estimate ")
modelEstimate
print("School's Average SAT Math Score")
...
Clearly there is a relationship between the number of students who will score high on one test at a school, either SAT or ACT, and the number of students who will score well on the other test. The correlation from above was 0.9, meaning it was very high While my initial thought was that this must just be because larger schools will have more students scoring above average on both test because they simply have more students who can, this next graph shows that the linear model for enrollment versus number of students scoring high on the ACT does not seem to be as strong as a correlation, explaining why we only had a pearson calculation of 0.5. 
...
Thus it is interesting to see that schools do not seem to prepare students for simply one of the two tests, but instead seem to either prepare students to do well both all tests, or not. And that just simply having more students is a weaker correlation to the number of students scoring high on a test compared to looking at the scores of a different test.
...
```{r   Urban_Counties with Correlations}
func <- function(xx)
{
